  ...
  <head>
    ...
    <script type="text/javascript"> // This should really be in a separate file
      ...
      function nameChanged (name) {
        $("u" + getRowNumber (name)) .checked = name.defaultValue != name.value;
        conditionallyEnableButton ("updRowNum[]", "updateButton");
      }
    
      function conditionallyEnableInsertButton() {
        $("insertButton") .disabled = $("ii").value=="" || $("in").value=="";
       }

      function insertIdChanged (id) {
        if (id.value.match ("^\\s*[0-9]*[1-9][0-9]*\\s*$")) {
          id.removeClassName ("invalid");
          $("idMessageInsert").textContent = "";
          $("idMessageInsert").parentNode.addClassName ("hidden");
        } else {
          id.addClassName ("invalid");
          $("idMessageInsert").textContent = "Number";
          $("idMessageInsert").parentNode.removeClassName ("hidden");
        }
            ...
            <tr>
              <td class="updDelBox">
                ...
                <input type     = "checkbox"
                       ...
                       onchange = "deleteBoxChanged (this);"
                />
              </td>
              <td class="updDelBox">
                <!-- Update checkbox -->
